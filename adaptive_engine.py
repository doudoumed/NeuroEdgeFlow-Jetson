...
LAPTOP_IP           = "10.0.20.10"      # laptop endpoint
...
class AdaptiveEngine(object):
    ...
    def __init__(self, poll_interval=POLL_INTERVAL_S, log_file=LOG_FILE,
        ...
        # Telemetry snapshot — populated each _tick, read by snapshot()
        self._snap_lock     = threading.Lock()
        self._last_snapshot = {
            "cpu_load": 0.0, "ram_usage": 0.0, "gpu_load": 0.0,
            "gpu_temp": 0.0, "rtt_ms": 0.0, "bandwidth_kbps": 0.0,
            "local_latency_ms": -1.0, "cloud_latency_ms": -1.0,
            "queue_depth": 0, "error_rate_pct": 0.0,
    ...
    def snapshot(self):
        """
        Return a thread-safe copy of the latest engine telemetry as a dict.

        Called by main_pipeline.py once per frame for unified CSV logging.
        Keys: cpu_load, ram_usage, gpu_load, gpu_temp, rtt_ms,
              bandwidth_kbps, local_latency_ms, cloud_latency_ms,
              queue_depth, error_rate_pct, network_ok, bandwidth_ok,
              fallback_locked.

        Safe to call before the first _tick — returns initialised zeros.
        """
    ...
    def update_cloud_latency(self, latency_ms):
        """Called safely by main_pipeline.py to update cloud inference latency"""
        self._cloud_latency = latency_ms
    ...
    def _get_hw_metrics(self):
        """Zero-dependency CPU/RAM/GPU fetch from /proc and /sys."""
        try:
            ...
            idle, total, ram = 0, 0, 0

        # GPU Load — Jetson TX2 exposes 0-1000 in /sys/devices/gpu.0/load
        gpu_load = 0.0
        for gpu_path in ["/sys/devices/gpu.0/load",
            ...
            try:
                with open(gpu_path, 'r') as f:
                    gpu_load = float(f.read().strip()) / 10.0   # 0-1000 → 0-100%
                break
            except (IOError, OSError, ValueError):
                continue

        # GPU Temperature — Jetson thermal zones
        gpu_temp = 0.0
        # Try the labelled thermal zone first
        try:
            for i in range(10):
                ...
                try:
                    ...
                    if 'gpu' in label:
                        with open(tz + "/temp", 'r') as f:
                            gpu_temp = float(f.read().strip()) / 1000.0  # mC → °C
                        break
                except (IOError, OSError, ValueError):
                    continue
        except:
        ...
        return (idle, total, ram, gpu_load, gpu_temp)

    def _control_loop(self):
        prev_i, prev_t, _, _, _ = self._get_hw_metrics()
        while self._running and not self._stop_event.is_set():
            try:
                # Calculate CPU delta
                curr_i, curr_t, ram, gpu_load, gpu_temp = self._get_hw_metrics()
                cpu = (1.0 - (curr_i - prev_i) / (curr_t - prev_t)) * 100.0 if curr_t != prev_t else 0.0
                prev_i, prev_t = curr_i, curr_t
                
                self._tick(cpu, ram, gpu_load, gpu_temp)
            except Exception as exc:
                log.error("Unhandled exception in control loop: %s", exc)
            self._stop_event.wait(timeout=self._poll_interval)

    def _tick(self, cpu_load=0.0, ram_usage=0.0, gpu_load=0.0, gpu_temp=0.0):
        self._poll_id += 1
        ts = time.strftime("%Y-%m-%d %H:%M:%S")
        ...
        conditions["gpu_load"] = gpu_load
        conditions["gpu_temp"] = gpu_temp
        ...
        if self._fallback_locked:
            ...
            _write_csv_row(self._log_file, {
                ...
                "gpu_load":        "{:.2f}".format(detail.get("gpu_load", 0.0)),
                "gpu_temp":        "{:.2f}".format(detail.get("gpu_temp", 0.0)),
                "prev_rtt_ms":     0,
                "rtt_delta":       0,
                "rtt_trend":       0,
        ...
        # ── 8. SNAPSHOT — store latest telemetry for snapshot() ──────────────
        with self._snap_lock:
            self._last_snapshot = {
                "cpu_load":         float(detail.get("cpu_load", 0.0)),
                "ram_usage":        float(detail.get("ram_usage", 0.0)),
                "gpu_load":         float(detail.get("gpu_load", 0.0)),
                "gpu_temp":         float(detail.get("gpu_temp", 0.0)),
                "rtt_ms":           float(detail.get("rtt_ms", 0.0)),
                "bandwidth_kbps":   float(detail.get("bandwidth_kbps", 0.0)),
                "local_latency_ms": float(self._local_latency),
                "cloud_latency_ms": float(self._cloud_latency),
                "queue_depth":      int(self._queue_depth),
                "error_rate_pct":   float(detail.get("error_rate_pct", 0.0)),
                "network_ok":       bool(detail.get("network_ok", False)),
                "bandwidth_ok":     bool(detail.get("bandwidth_ok", False)),
                "fallback_locked":  bool(self._fallback_locked),
            }
